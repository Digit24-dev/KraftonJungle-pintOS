...
/*
	각 요청의 크기(바이트 단위)는 2의 제곱수로 반올림되어 해당 크기의 블록을 관리하는 "디스크립터"에 할당된다.
	디스크립터는 free block의 list를 유지한다. free list가 비어있지 않으면, 목록에 있는 블록 중 하나를 사용하여 요청을 충족한다.
	
	그렇지 않으면, "arena"라고 불리는 새로운 메모리 페이지를 페이지 할당자에서 가져온다. (가용 페이지가 없으면 null 반환)
	새로운 아레나는 블록으로 나뉘어지며, 모든 블록이 디스크립터의 free list에 추가된다. 그런 다음 새로운 블록 중 하나를 반환한다.

	블록을 해제할 때, 해당 블록을 디스크립터의 자유 목록에 추가한다. 그러나 블록이 속해 있던 아레나에 사용 중인 블록이 더 이상 없다면,
	해당 아레나의 모든 블록을 자유 목록에서 제거하고 아레나를 페이지 할당자에게 반환한다.

	이 방식으로는 2kB보다 큰 블록을 처리할 수 없다. 그 이유는 그런 블록은 디스크립터와 함께 단일 페이지에 맞지 않기 때문이다.
	이러한 경우엔, 페이지 할당자를 사용하여 연속된 페이지를 할당하고, 할당된 블록의 아레나 헤더 시작 부분에 할당 크기를 기록하는 방식으로 처리한다.
*/
